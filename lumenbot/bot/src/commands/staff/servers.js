...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('servers')
    .setDescription('List all servers the bot is in and their waitlist status (Owner only)'),
  ...
  async execute(interaction, client) {
    ...
    for (const g of guilds) {
      ...
      const waitlisted = row?.waitlisted ? '✅ Waitlisted' : '❌ Not waitlisted';
      lines.push(`**${g.name}**\n└ ID: \`${g.id}\` • Members: ${g.memberCount} • ${waitlisted}`);
    }
