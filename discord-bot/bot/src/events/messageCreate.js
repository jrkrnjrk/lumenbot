...
module.exports = {
  ...
  async execute(message, client) {
    ...
    // ===== CUSTOM / TEXT COMMANDS (configurable prefix) =====
    if (message.guild && !message.author.bot) {
      const gset = await db.getGuildSettings(message.guild.id);
      let plugins = {};
      try { plugins = typeof gset?.plugins === 'string' ? JSON.parse(gset.plugins) : (gset?.plugins || {}); } catch {}
      const prefix = (gset?.command_prefix || '!').slice(0, 5) || '!';
      // Custom commands always work with the configured prefix (default !)
      // text_commands plugin enables prefix for built-in aliases later
      ...
      for (const [, g] of client.guilds.cache) {
        try {
          ...
          if (gs && (gs.whitelisted || gs.waitlisted) && gs.modmail_enabled !== false) {
            options.push({ label: g.name.slice(0, 100), value: g.id, description: `ID: ${g.id}`.slice(0, 100) });
          }
        } catch {}
      }

      if (options.length === 0) {
        await message.reply('No servers available for modmail (bot not shared, or modmail disabled).');
        return;
      }

      if (options.length === 1) {
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId(`modmail_server_${options[0].value}`).setLabel(`Open ticket in ${options[0].label.slice(0, 60)}`).setStyle(ButtonStyle.Success).setEmoji('✅'),
          new ButtonBuilder().setCustomId('modmail_cancel').setLabel('Cancel').setStyle(ButtonStyle.Secondary)
        );
        await message.reply({ content: 'Do you want to create a **modmail ticket**?', components: [row] });
        return;
      }

      // Multiple servers → select menu
      const { StringSelectMenuBuilder } = require('discord.js');
      const select = new StringSelectMenuBuilder()
        .setCustomId('modmail_select_server')
        .setPlaceholder('Choose a server for your ticket')
        .addOptions(options.slice(0, 25));
      const row = new ActionRowBuilder().addComponents(select);
      await message.reply({ content: 'Select the **server** you want to open a modmail ticket in:', components: [row] });
      return;
    }
